...
Register-ArgumentCompleter -CommandName $commands -ParameterName SpecialKeyInput -ScriptBlock {
    ...
    switch ($commandName) {
        'Set-VMKeystrokes' { 
           ...
           #Match character map objects that are upper or lowercase letters only and are more than a single character length
           $filter = {($_ -Match "([a-zA-Z]+$)" -or $_ -Match "[F]+[0-9]+$" -and $_.Length -gt 1)}
        }
        default { $filter = { $_ } }
    }
